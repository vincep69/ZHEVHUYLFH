...
<form method="post" action="medicament_upd.php">
	...
	<div class="col-xs-12 col-sm-12 col-md-6 col-lg-4">
		<div class="input-group">
		  ...
		  <?php
		  if ($_SESSION['grade']<50) {
		  		?><textarea type="text" name="medcompo" class="form-control" rows="1" value="" aria-describedby="basic-addon1" disabled><?php echo $data['MED_COMPOSITION']; ?></textarea><?php
		  	}else{
			  	if (isset($_GET['depotlegal'])) {
			  		?><textarea type="text" name="medcompo" class="form-control" rows="1" value="" aria-describedby="basic-addon1"><?php echo $data['MED_COMPOSITION']; ?></textarea><?php
			  	}else{
					?>
